...
def match_templates(df, waves, templates, raw):
    rows=[]
    for t in templates:
        tid=t["template_id"]; threshold=float(t.get("correlation_threshold",.85)); q=None
        if t.get("source")=="time_range":
            q=raw[(raw.timestamp>=pd.Timestamp(t["start"]))&(raw.timestamp<=pd.Timestamp(t["end"]))].pressure_bar.to_numpy()
        elif t.get("source")=="waveform_csv":
            q=pd.read_csv(t["path"])["pressure_bar"].to_numpy()
        ...
        for i,x in enumerate(waves):
            ...
            if corr>=threshold:
                r=df.iloc[i].to_dict(); r.update(template_id=tid,similarity=corr); rows.append(r)
...
def main():
    ...
    f,w=idle_windows(raw); f.to_csv(out/"idle_window_features.csv",index=False)
    ...
    templates=load_templates(a.templates); matches=match_templates(f,w,templates,raw); matches.to_csv(out/"template_matches.csv",index=False)
    if len(matches):
        m=matches.assign(date=matches.start_time.dt.date).groupby(["date","template_id"]).agg(matches=("similarity","count"),mean_similarity=("similarity","mean"),mean_amplitude=("range_bar","mean")).reset_index(); m.to_csv(out/"template_daily_summary.csv",index=False)
    print(f"idle_windows={len(f)} templates={len(templates)} matches={len(matches)} output={out}")
